...
describe('entity detail image descriptor boundary', () => {
  ...
  it('loads and strictly parses the Task33 gallery descriptor response', () => {
    ...
    expect(detailSource).toContain('catch { return { requestId, descriptors: [], failed: true } }')
    expect(detailSource).toContain('const galleryPartial = computed')
    expect(detailSource).toContain(':retry="refreshGallery"')
    expect(detailSource).toContain('describeEntityImages(entity.value || {})')
    expect(detailSource).not.toContain('normalizeRenderableImageUrl(raw)')
    expect(detailSource).not.toMatch(/legacy[\s\S]*source_class:\s*'ai-generated'/)
