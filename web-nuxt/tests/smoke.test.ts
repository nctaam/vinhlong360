...
describe('UserCP regressions', () => {
  ...
  it('homepage hero uses intent-led copy and safer visual affordances', () => {
    ...
    const decisions = src('components/home/HomeDecisionLedger.vue')
    const categories = src('components/home/HomeCategoryIndex.vue')
    const presentation = src('utils/homeNocturnePresentation.ts')
    ...
    expect(page).toContain('JourneyActionRail')
    expect(decisions).toContain('Hôm nay bạn muốn bắt đầu thế nào?')
  ...
  it('default layout mounts client widgets after hydration', () => {
    ...
    const themeControl = src('components/shell/ThemeModeControl.vue')
    ...
    expect(layout).toContain('<SearchAutocomplete class="topbar-search public-shell-search" />')
    expect(layout).toContain('<ShellThemeModeControl />')
    expect(themeControl).toContain('Nocturne')
    expect(themeControl).toContain('Nền sáng dễ đọc')
  ...
  it('data validation report exposes text quality counters', () => {
    ...
    // Counter nằm trong dict report-data (định nghĩa TRƯỚC khối print, in qua loop
    // generic) → check cả file `validator`, không phải reportBlock (đã hẹp sau refactor).
    expect(validator).toContain('"near_one_way_edges"')
    expect(validator).toContain('"coordinate_clusters_precise"')
    expect(validator).toContain('"text_repeated_sentence"')
    expect(validator).toContain('"text_bad_sentence_join"')
    expect(validator).toContain('"text_excessive_length"')
