...
const baselineSources = () => ({
  ...
  'assets/css/catalog.css': `${repeatedLegacyConsumers(50)}\n:root {\n  --AREA-rgb: var(--primary-rgb);\n}\n`,
  'assets/css/detail.css': repeatedLegacyConsumers(39),
})
...
describe('tri-region color debt ratchet', () => {
  ...
  it('counts the existing catalog custom-property dependency in the exact accepted baseline', () => {
    ...
    expect(result.status).toBe(0)
    expect(result.stdout).toContain('assets/css/catalog.css: rawHex 0/5, legacyPrimary 51/51')
    expect(result.stdout).toContain('shared catalog.css + detail.css: rawHex 0/25, legacyPrimary 90/90')
  })
  ...
  it('rejects a manifest ceiling increase even when the measured source remains below it', () => {
    const manifest = cloneManifest()
    manifest['assets/css/catalog.css']!.legacyPrimary = 52
    const result = runChecker(createFixture({ manifest }))
