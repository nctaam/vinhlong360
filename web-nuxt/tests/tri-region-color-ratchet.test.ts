...
const approvedBudgets = {
  ...
  'assets/css/catalog.css': { rawHex: 0, legacyPrimary: 0 },
  'assets/css/detail.css': { rawHex: 0, legacyPrimary: 0 },
}
...
const baselineSources = () => ({
  ...
  'assets/css/catalog.css': repeatedLegacyConsumers(0),
  'assets/css/detail.css': repeatedLegacyConsumers(0),
})
...
describe('tri-region color debt ratchet', () => {
  ...
  it('counts the existing catalog custom-property dependency in the exact accepted baseline', () => {
    ...
    expect(result.status).toBe(0)
    expect(result.stdout).toContain('assets/css/catalog.css: rawHex 0/0, legacyPrimary 0/0')
    expect(result.stdout).toContain('shared catalog.css + detail.css: rawHex 0/0, legacyPrimary 0/0')
  })
  ...
  it('rejects a manifest ceiling increase even when the measured source remains below it', () => {
    const manifest = cloneManifest()
    manifest['assets/css/catalog.css']!.legacyPrimary = 1
    const result = runChecker(createFixture({ manifest }))
