...
describe('SEO & Editorial Craft Guardrails', () => {
  ...
  it('Regional hub has BreadcrumbList JSON-LD, AdministrativeArea schema, vector cross-links, safeJsonLd, and Twitter card', () => {
    ...
    expect(area).toContain('<CatalogCrossLinks')
    expect(area).toContain("icon: 'sprout'")
    expect(area).toContain("icon: 'fruit'")
    expect(area).toContain("icon: 'home'")
  ...
  it('Seasonal hub uses vector icons for B2B callout, calendar, and cross-links without raw emojis', () => {
    ...
    expect(season).toContain('<CatalogCrossLinks />')
    const crossLinks = doc('components/CatalogCrossLinks.vue')
    expect(crossLinks).toContain("icon: 'fruit'")
    expect(crossLinks).toContain("icon: 'star'")
    expect(crossLinks).toContain("icon: 'leaf'")
    expect(crossLinks).toContain("icon: 'bowl'")
  ...
  it('Itinerary index hub has CollectionPage schema with safeJsonLd, IconLine plus, vector cross-links, and no duplicate breadcrumbs', () => {
    ...
    expect(itinerary).toContain('<IconLine name="plus"')
    expect(itinerary).not.toContain('+ Tự tạo lịch trình')
    expect(itinerary).toContain('<CatalogCrossLinks />')
    const crossLinks = doc('components/CatalogCrossLinks.vue')
    expect(crossLinks).toContain("icon: 'leaf'")
    expect(crossLinks).toContain("icon: 'home'")
    expect(crossLinks).toContain("icon: 'map'")
    expect(crossLinks).toContain("icon: 'fruit'")
  ...
  it('Accommodation and Events hubs have ogUrl, twitterCard, and clean vector icons without raw emojis', () => {
    ...
    expect(suKien).toContain('<CatalogCrossLinks />')
    const crossLinks = doc('components/CatalogCrossLinks.vue')
    expect(crossLinks).toContain("icon: 'lantern'")
    expect(crossLinks).toContain("icon: 'leaf'")
    expect(crossLinks).toContain("icon: 'calendar'")
    expect(crossLinks).toContain("icon: 'map'")
