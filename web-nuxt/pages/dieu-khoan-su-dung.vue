<template>
  <section class="legal-page about-page">
    <Breadcrumb :items="[{ label: 'Trang chủ', to: '/' }, { label: 'Điều khoản sử dụng' }]" :json-ld="true" />
...
const termsSchema = computed(() => ({
  ...
  '@type': 'WebPage',
  name: doc.value.title || 'Điều khoản sử dụng — vinhlong360',
  description: doc.value.seo_description || 'Điều khoản sử dụng và quy định dịch vụ trên vinhlong360.',
  url: canonicalUrl('/dieu-khoan-su-dung'),
  inLanguage: 'vi',
}))
...
useSeoMeta({
  ...
  ogUrl: () => canonicalUrl('/dieu-khoan-su-dung'),
...
useHead(() => ({
  ...
  script: [
    { type: 'application/ld+json', innerHTML: safeJsonLd(termsSchema.value) },
  ],
}))
</script>
