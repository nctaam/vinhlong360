  ...
  <div>
    <div class="admin-head-row">
      <div>
        ...
        <h1>Thông báo</h1>
        <p class="cs-subtitle">Banner thông báo trên đầu trang</p>
      </div>
    </div>
    ...
    <Transition name="cs-fade">
      <div v-if="!loading" class="cs-form-wrap">
        <AdminSettingsForm :category="'announcements'" :fields="fields" @saved="reload" />
      </div>
    </Transition>
  </div>
</template>
...
async function reload() {
  loading.value = true
  try {
    const r = await $fetch<any>('/admin-api/site-settings/announcements', { headers: authHeaders() })
