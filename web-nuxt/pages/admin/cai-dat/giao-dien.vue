  ...
  <div>
    <div class="admin-head-row">
      <div>
        ...
        <h1>Giao diện</h1>
        <p class="cs-subtitle">Tùy chỉnh màu sắc toàn trang (để trống = dùng mặc định)</p>
      </div>
    </div>
    ...
    <Transition name="cs-fade">
      <div v-if="!loading" class="cs-form-wrap">
        <AdminSettingsForm :category="'theme'" :fields="fields" @saved="reload" />
      </div>
    </Transition>
  </div>
</template>
...
async function reload() {
  loading.value = true
  try {
    const r = await $fetch<any>('/admin-api/site-settings/theme', { headers: authHeaders() })
