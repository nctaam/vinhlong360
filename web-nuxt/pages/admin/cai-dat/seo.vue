  ...
  <div>
    <div class="admin-head-row">
      <div>
        ...
        <h1>SEO</h1>
        <p class="cs-subtitle">Tiêu đề, mô tả, theme color mặc định</p>
      </div>
    </div>
    ...
    <Transition name="cs-fade">
      <div v-if="!loading" class="cs-form-wrap">
        <AdminSettingsForm :category="'seo'" :fields="fields" @saved="reload" />
      </div>
    </Transition>
  </div>
</template>
...
async function reload() {
  loading.value = true
  try {
    const r = await $fetch<any>('/admin-api/site-settings/seo', { headers: authHeaders() })
