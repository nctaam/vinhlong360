  ...
  <div>
    <div class="admin-head-row">
      <div>
        ...
        <h1>Thương hiệu</h1>
        <p class="cs-subtitle">Tên trang, slogan, ảnh Open Graph</p>
      </div>
    </div>
    ...
    <Transition name="cs-fade">
      <div v-if="!loading" class="cs-form-wrap">
        <AdminSettingsForm :category="'branding'" :fields="fields" @saved="reload" />
      </div>
    </Transition>
  </div>
</template>
...
useHead({ title: 'Thương hiệu — Admin' })
...
async function reload() {
  loading.value = true
  try {
    const r = await $fetch<any>('/admin-api/site-settings/branding', { headers: authHeaders() })
