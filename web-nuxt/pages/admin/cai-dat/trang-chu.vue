  ...
  <div>
    <div class="admin-head-row">
      <div>
        ...
        <h1>Trang chủ</h1>
        <p class="cs-subtitle">Hero text, pills, quick links, chatbot CTA</p>
      </div>
    </div>
    <div v-if="loading" class="cs-skeleton">
      <div class="cs-skel-row" v-for="n in 5" :key="n"><div class="cs-skel-label"></div><div class="cs-skel-input" :class="{ 'cs-skel-tall': n > 3 }"></div></div>
    </div>
    <Transition name="cs-fade">
      <div v-if="!loading" class="cs-form-wrap">
        <AdminSettingsForm :category="'homepage'" :fields="fields" @saved="reload" />
      </div>
    </Transition>
  </div>
</template>
...
async function reload() {
  loading.value = true
  try {
    const r = await $fetch<any>('/admin-api/site-settings/homepage', { headers: authHeaders() })
