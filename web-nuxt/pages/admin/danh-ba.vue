<template>
  <div>
    ...
    <p class="muted">Nhập cơ quan công vụ (UBND/công an/...) theo xã/phường. <strong>Bắt buộc khai nguồn chính thống</strong> — không tự bịa địa chỉ/SĐT.</p>
    ...
    <h2 style="font-size:1.1rem; margin:20px 0 8px">Đã có ({{ facilities.length }})</h2>
    <table v-if="facilities.length" class="tbl">
      <thead><tr><th>Cơ quan</th><th>Liên hệ</th><th>Nguồn</th><th></th></tr></thead>
      <tbody>
        <tr v-for="e in facilities" :key="e.id">
          <td><strong>{{ e.name }}</strong><br><small class="muted">{{ kindLabel(e) }} · {{ placeName(e) }}</small></td>
          <td><small>{{ attr(e,'phone') }}<br>{{ attr(e,'address') }}</small></td>
          <td><small class="muted">{{ e.source?.url || e.source?.title || '—' }}</small></td>
          <td><button class="btn btn-sm btn-ghost danger" @click="del(e)">Xóa</button></td>
        </tr>
      </tbody>
    </table>
...
async function loadFacilities() {
  facilities.value = await $fetch<any>('/api/facilities').then((r: any) => r.facilities || []).catch(() => [])
}
...
.card-form { border: 1px solid rgba(0,0,0,.1); border-radius: 10px; padding: 16px; margin-top: 12px; }
