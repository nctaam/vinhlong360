...
const {
  ...
  showToast,
  confirm,
  navigateTo: (to: string) => { void navigateTo(to) },
})
...
.sf-input {
  ...
  border-radius: var(--radius-surface); background: var(--bg); color: var(--ink-900);
...
.sf-username-row { display: flex; align-items: center; gap: 0; border: 1px solid var(--border-input); border-radius: var(--radius-surface); overflow: hidden; }
...
.toggle { appearance: none; width: 40px; min-width: 44px; height: 22px; background: var(--muted); border-radius: 11px; position: relative; cursor: pointer; transition: background .25s var(--ease-out); flex-shrink: 0; min-height: 44px; padding: 11px 0; box-sizing: content-box; margin: 0; }
