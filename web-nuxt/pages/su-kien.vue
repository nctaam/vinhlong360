<template>
  <div class="page events-page">
    ...
    <section v-if="upcoming.length" class="block reveal">
      ...
      <ul class="ledger" aria-label="Sự kiện sắp diễn ra, theo thứ tự gần nhất">
 ...
 * RFC 5545 §3.6.1) has a single implementation shared with le-hoi and the per-row button.
 */
function downloadIcalBulk() {
  ...
  downloadIcsBundle(items, 'su-kien-sap-toi.ics')
}
...
<style>
.catalog-hero.cat-event { background: linear-gradient(135deg, rgba(var(--accent-rgb), .08) 0%, rgba(183, 110, 60, .06) 100%); }
