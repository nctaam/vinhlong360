...
.new-post-thread-hint {
  ...
  background:
    linear-gradient(90deg, transparent, var(--color-material-river) 15%, var(--color-material-amber) 85%, transparent) bottom/100% 1.5px no-repeat,
    rgba(var(--color-warning-rgb), .05);
  transition: color .25s var(--ease-out), background-color .25s var(--ease-out);
  animation: hint-settle .4s var(--ease-out-expo) both;
}
.new-post-thread-hint:hover { color: var(--ink); background-color: rgba(var(--color-warning-rgb), .09); }
...
.dark .new-post-thread-hint { background-color: rgba(var(--color-warning-rgb), .08); }
.dark .new-post-thread-hint:hover { background-color: rgba(var(--color-warning-rgb), .13); }
