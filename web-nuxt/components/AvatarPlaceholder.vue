<template>
  ...
  <svg v-else viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg" class="avatar-placeholder" aria-hidden="true">
    <rect width="80" height="80" rx="40" fill="var(--primary-light)" />
    <circle cx="40" cy="30" r="14" fill="rgba(var(--white-rgb), .35)" />
    <ellipse cx="40" cy="68" rx="22" ry="18" fill="rgba(var(--white-rgb), .25)" />
    <text v-if="initial" x="40" y="38" text-anchor="middle" fill="var(--white)" font-size="28" font-weight="800" font-family="system-ui">{{ initial }}</text>
  </svg>
</template>
