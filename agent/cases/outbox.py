...
def _claim(database, conn, now: datetime, limit: int) -> list[dict]:
    ...
    return [dict(database._row_to_dict(row)) for row in rows]
...
def dispatch_case_outbox(*, now: datetime, limit: int = 100) -> DispatchSummary:
    ...
    with database._conn(commit_on_success=False) as conn:
        for item in _claim(database, conn, now, limit):
            claimed += 1
            ...
            result = _PROVIDER.send(contact, message, delivery_key=delivery_key(outbox_id, attempts))
