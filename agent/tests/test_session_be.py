...
class TestPhase8SessionCleanup:
    ...
    # ĐÃ GỠ `test_session_cleanup_purges_deleted_users_after_grace...`.
    # Nó khẳng định session-cleanup tự xoá cứng tài khoản — kiến trúc của nhánh NP-1,
    # trái với `test_account_erasure_is_separate_from_session_cleanup` trong chính file
    # này và với hai guard nguồn. Xem ghi chú dài trong agent/scheduler.py.
...
class TestPhase9ErrorInfoLeaks:
    ...
    def _get_admin_src(cls):
        if cls._admin_src is None:
            ...
            + (Path(__file__).resolve().parent.parent / "community" / "admin_api.py").read_text(encoding="utf-8") + chr(10)  # mien community-admin sang goi rieng 2026-08-29
            + (Path(__file__).resolve().parent.parent / "siteops" / "admin_api.py").read_text(encoding="utf-8"))  # mien van hanh site (backup/ops/site-settings/announcements) sang goi rieng 2026-08-29 (lat 3)
