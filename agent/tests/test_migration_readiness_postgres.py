...
def test_fresh_migration_chain_reaches_release_readiness(fresh_migrated_database):
    ...
    # Migration 081 chủ duyệt (commit 834a8d25) — "080 LOCKED" đã gỡ có chủ đích;
    # PG_REQUIRED_SCHEMA_VERSION = 81 tại agent/database.py:193. Giữ literal.
    assert [migration.version for migration in applied][-3:] == [79, 80, 81]
    ...
    assert status["schema_version"] == 81
    assert status["required_schema_version"] == 81
