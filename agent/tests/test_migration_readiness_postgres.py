...
def test_fresh_migration_chain_reaches_release_readiness(fresh_migrated_database):
    ...
    # toàn quyền máy này 2026-08-29). PG_REQUIRED_SCHEMA_VERSION = 82. Giữ literal.
    assert [migration.version for migration in applied][-3:] == [80, 81, 82]
    ...
    assert status["schema_version"] == 82
    assert status["required_schema_version"] == 82
