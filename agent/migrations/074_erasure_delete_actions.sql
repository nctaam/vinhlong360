-- Migration 074: register user FK actions and nullable audit/decision actors.
...
BEGIN
    IF to_regclass(format('%I.users', schema_name)) IS NULL THEN
        RAISE EXCEPTION 'users table is required before migration 074';
    END IF;
...
VALUES ('agent', 74, '074_erasure_delete_actions.sql', NOW())
