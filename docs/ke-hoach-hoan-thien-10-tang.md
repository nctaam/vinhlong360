...
**ĐỢT 3 — quét nốt phần khả-thi (2026-06-23):** ✅ **TC-02/TC-10 chat tests** (test_chat_tools.py — bắt+sửa BUG thật `logger.warning`→`warn` server.py, đã redeploy) · **BreadcrumbList opt-in** (Breadcrumb prop jsonLd, bật su-kien/luu-tru) · **confirm-dialog in-app** (useConfirm + ConfirmDialog, thay confirm()/alert() ở builder/itinerary-list/EntityReviews/UserMenu).
