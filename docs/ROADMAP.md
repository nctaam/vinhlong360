...
- **(2026-06-23) 📋 KẾ HOẠCH HOÀN THIỆN 10 TẦNG → `docs/ke-hoach-hoan-thien-10-tang.md`** — quét sâu toàn hệ thống 6 agent song song (public/UGC/admin/backend-core/AI-ops/docs-test). Khung 10 tầng + backlog đã-verify file:line (10 P0, 20 P1, 12 P2) + lộ trình 8 đợt (A→H) + DoD/tầng. **P0 nổi bật (bug thật):** claim-flow vỡ (`?ref=claim`↔`?claim=`), `/dang-nhap`+`/cai-dat` 404 (phễu auth chết), tel: regex bug (`/\\./`), role-change query-param (422), theme CSS-injection, session token plaintext, comment không kiểm duyệt, phone PII leak, ai.vue confirm cost/§B8, guard nội-dung chỉ khuyến-cáo (B6). **Tin tốt (verify, KHÔNG sửa):** §B8 gating chắc, không XSS sink (mdLite an toàn), backend admin-auth enforce đúng, SQL parameterized. Chờ chủ duyệt thứ tự thực thi. **+ PHẦN II (Wave 2 đào sâu):** verdict P0 (xác nhận 6/10, hạ-cấp 5/9/10) + **10 P0 MỚI verified** (profile user vỡ, dynamic-agent no-auth, SSRF approve-image, 2 DB CHECK→500, login no-rate-limit, website href XSS, map-popup HTML-injection ×3, bookmark key, builder coords, /api/report collision) + data-contract (4 envelope/4 pagination) + KB định lượng (catch-all p-long-chau 247, 5 ward rỗng, 38 mis-type, 9 dup, 643 approx) + CWV (homepage cache, prerender 1700 detail, hero 217KB→WebP) + chat test-matrix (TC-02×14/TC-10×13, `_is_error_reply` false-positive). Refuted: UGC IDOR, 29-ward-rỗng (thật 5).
...
- **[Data/§B7] 37/162 entity `type='place'` phân-loại nhầm** (ngân-hàng/quán ăn, `level=None`) → lọt `/api/places` + `/api/stats.places=162` (thật chỉ 125 xã/phường: 89 xã+35 phường+1 tỉnh) → **trang chủ hiện "162 xã phường" SAI** (danh-bạ lọc theo level nên đúng 125). Cần reclass 37 mục — DATA op, cần chủ duyệt + backup (§B7). Tạm có thể đưa vào `/admin/chua-phan-loai`.
- **[Med/Perf] `database.count_relationships`** chạy lại full JOIN rồi `len()`; `/api/entities/{id}` gọi cả count + get → query đắt 2× mỗi request (9303 rel). Đổi sang `SELECT COUNT(*)` (khớp filter rel_type/include_near).
- **[Med/Consistency] 9 chỗ FE còn `$fetch('/api/...')` SSR**: `dia-diem/[id]`, `bai-viet/[id]`, `xa-phuong/[id]`, `nguoi-dung/[id]`, `lich-trinh-chia-se/[id]`, `components/NearbyEntities.vue`, `tim-kiem.vue×3`, `plugins/site-overrides.ts`+`composables/useSiteSettings.ts`. Agent verify prod HIỆN render OK → rủi-ro tiềm-ẩn, không phải lỗi đang xảy ra; chuyển apiFetch để đồng-bộ + chống tái-diễn (đánh-đổi: +1 hop nội-bộ khi SSR).
...
- **[Low] `notifications.toggle_follow`/report-ugc/block** không kiểm target tồn-tại → bản-ghi rác. Thêm guard `db.get_entity`.
