...
- 🟠 **`detail.css` (48KB) → CÒN, surgery nhiều bước (cần QA thị giác có backend):** chứa CSS component dùng-chung — cách bài bản = chuyển CSS vào chính component (co-locate, nạp theo component) rồi đẩy phần cover-trang về 3 trang chi tiết:
  - Tách sạch (class chỉ-trong-1-component): `EntityReviews` (`.reviews-*/.review-*/.ri-*/.rf-*`), `NearbyEntities` (`.nearby-*`) → `<style>` trong component.
  - Cần xử khéo (dùng nhiều nơi): lightbox `.lb-*/.lightbox` (ImageGallery+PostCard+dia-diem) → file shared import 2 component; `.breadcrumb/.bc-back` (Breadcrumb.vue + nhiều page) → giữ global hoặc vào Breadcrumb.vue.
  - Cover-trang `.detail-cover/.dc-*/.highlights/.hl` → `<style src>` ở dia-diem/[id], xa-phuong/[id], lich-trinh/[id].
  - **Mỗi bước: build + verify chunk (như events) + QA thị giác trang chi tiết & cong-dong (lightbox) với backend chạy.** `base.css` 71KB (lớn nhất) = audit prune riêng (Backlog).
