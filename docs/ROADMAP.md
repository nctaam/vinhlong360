...
- ✅ **P1 egress observability đã giải local:** commit `f2b50bbb` thêm đúng một warning đã sanitize tại boundary `PinnedHTTPClient.get()` cho `blocked_address`, `peer_mismatch`, `redirect_policy`; các consumer hiện dùng context cố định `admin_image_review`, `auto_learn`, `crawler`, `geocode`, `quality_burst`. Exception typed và hành vi consumer giữ nguyên. Commit test-hygiene `8e4bf9be` dọn leak `_draining` giữa integration/admin tests và selector chat lỗi thời; fresh coverage baseline đạt `8726 passed, 66 skipped, 26 deselected, 1 xfailed`. Review-fix `15f7124a` khóa mutation bỏ catch auto-learn phải đỏ nếu raw URL warning quay lại.
...
- **Bằng chứng geocode gate (2026-08-04):** regression RED trước implementation xác nhận direct `requests.get`, thiếu import/policy/audit context và registry còn coi geocode là unpinned; GREEN sau migration đạt focused geocode+registry `19 passed`, broad pinned + mapped consumers + geocode `351 passed in 38.84s`, changed-file Ruff sạch, `git diff --check` sạch và `run_hard.py --all` đạt `hard=0`, ratchet không tăng. `python -m ruff check .` vẫn báo 3 `F401` có sẵn trong `tests/checks/test_erasure_lifecycle_complexity.py`; file đó không đổi và cần commit test-hygiene riêng. Không chạy lại monolithic backend cho commit nhỏ này; baseline gần nhất vẫn là evidence crawler ngay trước đó.
- **Residual egress thật còn lại:** `tests/test_pinned_http_consumers.py::KNOWN_UNPINNED_FETCHERS` tiếp tục ghi nhận `realtime.get_weather`, `scheduler._digest_send`, `scheduler._send_telegram_admins` (2 caller cuối là POST). Production behavior vẫn chưa được quan sát vì chưa có deployment được uỷ quyền riêng.
