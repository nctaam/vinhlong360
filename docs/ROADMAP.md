...
- **Bằng chứng final gate (2026-07-27, revision `dab4877163280a6476180e0ad285280e405af1b4`, đo thật):** focused pinned suite `303 passed in 22.36s`; frontend `npm test` lần đầu exit `1` (`1 failed | 36 passed` file; `906 passed | 6 skipped`) do Nuxt setup hook timeout ở `tests/entityStory.test.ts`, không đổi code/test rồi rerun exact một lần → exit `0`, `37` files/`912` tests passed in `28.66s`; typecheck exit `0`, build exit `0` (`746 modules`, `6.45 MB`, manifest đúng revision); `run_hard.py --all` → exit `0`, `hard=0`, ratchet không tăng; `git diff --check` → exit `0`; official bounded backend → exit `0` in `6901.2s`, Phase A `8633 passed, 58 skipped, 111 deselected, 1 xfailed` in `1152.25s`, Phase B `284 passed, 19 skipped` in `5739.98s`.
- **Residual egress thật:** blocked destination/peer mismatch/redirect denial vẫn không có operational log; consent gate kiểu redirect về cùng URL sau `Set-Cookie` vẫn không tương thích vì client cố ý không có cookie jar. `tests/test_pinned_http_consumers.py::KNOWN_UNPINNED_FETCHERS` tiếp tục ghi nhận các caller bị spec loại trừ: `crawler.fetch_page`, `geocode._query_nominatim`, `realtime.get_weather`, `scheduler._digest_send`, `scheduler._send_telegram_admins` (2 caller cuối là POST).
