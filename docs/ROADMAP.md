...
- ✅ **P1 shared pinned outbound HTTP client:** `agent/pinned_http.py` sở hữu per-hop public DNS policy, exact-sockaddr dialing, peer verification, TLS hostname/SNI preservation và manual redirect validation cho 3 đường fetch: admin image review, auto-learn, GPT-5.5 quality-burst. `tests/test_pinned_http_consumers.py` khoá registry — 3 hàm đã map không còn gọi `httpx.get()`/`requests.get()` trực tiếp (grep repo-wide: 0 hit). Không push/deploy.
...
- **Bằng chứng gate (2026-07-27, đo thật, KHÔNG làm tròn):** focused P1 suite 972 passed/8 skipped. Repo-wide coverage `2 failed, 8532 passed, 58 skipped, 1 xfailed` (25:46); `run_hard.py --all` → `hard=0, ratchet không tăng` (exit 0); `run_backend_regression --deadline-seconds 7000` phase A `2 failed, 8532 passed` (24:23) → **exit 1**. **2 fail đó KHÔNG phải của tranche này**: `test_repository_has_no_duplicate_canonical_artifacts` + `test_unpacked_release_loaders_read_exact_packaged_bytes`, gây bởi worktree tạm của session Claude song song dưới `.claude/worktrees/`. Gọi thẳng scanner để chứng minh: `find_duplicate_artifacts(root)` → 2 trùng, **cả 2 nằm trong `.claude/worktrees/`**, **0 trùng trong cây được track**. Chưa có lần chạy exit 0 sạch nào — cần chạy lại khi không còn worktree song song. KHÔNG sửa scanner/test để làm xanh.
- **Residual egress debt:** streaming body/decompression bounds và whole-chain monotonic deadline vẫn thuộc Workstream 10. `tests/test_pinned_http_consumers.py::KNOWN_UNPINNED_FETCHERS` ghi nhận 5 caller chưa pin: `crawler.fetch_page`, `geocode._query_nominatim`, `realtime.get_weather`, `scheduler._digest_send`, `scheduler._send_telegram_admins` (2 cái cuối là POST — client cố ý chỉ GET nên không migrate được nếu không nới contract).
