...
- **CHƯA làm (Tầng-1 còn lại — cần duyệt; lưu ý CI chỉ chạy sau H3/remote):** (#4) route-render smoke CI (bắt lớp SSR-500 như /danh-ba, /theo-mua từng dính); (#5) typecheck `vue-tsc` + ESLint gate (chưa có script nào; 113 `: any`); (#6) `pip-audit`/`npm audit`/`gitleaks` trong CI.

- **(2026-06-21) 🟡 MED — Flaky test-isolation phụ thuộc thứ tự collection (CÒN)**: `tests/test_knowledge.py` (`test_search_entities_basic`, `test_places` → `assert 0 > 0`, KB rỗng) + `tests/test_tracing.py` (6 `test_yields_none`) ĐỎ khi chạy full suite với **path tường minh** (`pytest <root_path>`), nhưng XANH khi (a) chạy cô lập, (b) chạy baseline tài liệu từ root (`pytest` không path → 1189 passed). Nguyên nhân: một test làm rỗng `knowledge` global / đặt trạng thái `tracing` mà không khôi phục → kết quả phụ thuộc thứ tự. Đề xuất: autouse fixture trong `conftest` khôi phục `knowledge` (reload) + reset tracing state (như đã làm cho `test_retrieval_eval`); cân nhắc `pytest-randomly` để lộ sớm. **KHÔNG yếu đi assertion** (§3.4).
