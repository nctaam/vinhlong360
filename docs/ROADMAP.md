...
- **D2 (🤔 CẦN CHỦ DỰ ÁN QUYẾT — task đề xuất):** Nhóm OCOP cần *bán*, không chỉ *được thấy* (rủi ro cốt lõi của showcase-only). Đề xuất dung hòa **vẫn trong khuôn §1.4**: thêm **link-out** ra Zalo/sàn TMĐT/website chủ thể trên trang chi tiết product (dẫn-khách ra ngoài, **KHÔNG** giỏ hàng/booking/thanh toán on-site). Codeable nhỏ (1 field `attributes.shopUrl`/`marketplaceUrl` + nút "Mua ở…"). **CHỜ DUYỆT** vì đụng ranh giới định vị sản phẩm.
