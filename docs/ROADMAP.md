...
- **(2026-06-14) Design System (kế hoạch `docs/design-system-plan.md`)** — từ deep-research "thiết kế giao diện" (chủ duyệt). Giữ CSS thuần + tokens (KHÔNG Tailwind), giữ hệ màu hiện tại. 5 bước additive: ~~(1) hệ thống hóa tokens 3 tầng + thang typography/spacing 8pt~~ ✅ XONG (commit này: `variables.css` primitive→semantic, giữ alias cũ, 0 đổi pixel, build pass); (2) audit tương phản WCAG AA + target 44px; (3) self-host font `@nuxt/fonts` + ảnh `@nuxt/image` WebP/AVIF; (4) dark mode nút toggle `@nuxtjs/color-mode`; (5) JSON-LD Schema.org (GEO). Bước 2–5 chờ duyệt làm tiếp.
