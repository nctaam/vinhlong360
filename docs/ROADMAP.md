...
### 🎨 Tầng-2 CWV (2026-06-21) — điều tra, CHƯA thực thi
- **🟠 `entry.css` 202KB (gzip 33.5KB) tách bundle — refactor lớn, cần duyệt scope.** Đo: 8 file global (`base` 71KB, `detail` 48KB, `components` 40KB, `catalog` 39KB, `cards` 20KB, `events` 10.5KB, `variables` 9.8KB, `dark-overrides` 7KB; raw 246KB). Ứng viên route-specific = `detail`+`catalog`+`events` (~98KB, ~40%). **NHƯNG** các file này **trộn class dùng-chung với route-specific** (vd `detail.css` chứa `.breadcrumb`/`.bc-back`; `catalog.css` chứa `.chip`/`.search-row`/`.controls`) → tách naive theo file sẽ **vỡ breadcrumb/chip ở trang khác**. Muốn an toàn: tách từng class shared↔route, QA preview toàn site. Đúng đánh giá "refactor lớn" cũ → giữ Backlog tới khi duyệt làm bài bản.
