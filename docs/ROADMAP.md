...
- 🟠 **`detail.css` (48KB) → đang tách dần (co-locate CSS component, nạp theo component):**
  ...
  - ⏸ **CÒN (cần QA thị giác có backend):** lightbox `.lb-*/.lightbox` (ImageGallery+PostCard+dia-diem → file shared import 2 component); `.breadcrumb/.bc-back` (Breadcrumb.vue + nhiều page → vào Breadcrumb.vue hoặc giữ global); cover-trang `.detail-cover/.dc-*/.highlights/.hl` → `<style src>` ở dia-diem/[id], xa-phuong/[id], lich-trinh/[id].
  - `base.css` 71KB (lớn nhất) = audit prune unused riêng (Backlog).
  - **Mỗi bước còn lại: build + verify chunk + QA thị giác (dark/reduced-motion) trang chi tiết & cong-dong (lightbox) với backend chạy.**
