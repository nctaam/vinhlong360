...
- ⚠️ **CHƯA sửa — cùng họ lỗi, còn tiềm ẩn:** `test_resilience.py` gán thẳng `knowledge._entities` / `_relationships` / `_itineraries` / `_data_source` mà KHÔNG khôi phục (`TestKnowledgeSearchEdgeCases._seed_entities` ~dòng 643, `TestKnowledgeHealthCheck` ~dòng 1462 và ~1476 — test sau cùng để lại knowledge RỖNG vĩnh viễn cho worker đó). Chưa quan sát được fail nào do nó, nhưng là nguồn flaky y hệt. **Cố ý không sửa kèm:** fixture snapshot/restore knowledge toàn cục có thể làm ĐỎ các test đang xanh vốn (vô tình) dựa vào state rò — làm mù là vi phạm §3.3. Cần task riêng: đo trước xem test nào phụ thuộc, rồi mới thêm fixture.
