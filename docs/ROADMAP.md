...
- **(2026-06-13) GĐ10 phần còn lại — chưa làm**: (a) 10.1 hybrid prerender (cần pipeline build có backend); (b) QA browser map clustering (env có ndaMapKey); (c) sửa mojibake `admin/data-quality.vue`; (d) siết `any` (146 chỗ); ~~(e) rà clickable `<div>` toàn site → `<button>`~~ ✅ XONG (commit 1626dd8: index card→NuxtLink, tao-lich-trinh→button; 4 div @click.self còn lại là backdrop hợp lệ).
