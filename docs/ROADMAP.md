...
- **(2026-06-22) 🔬 AUDIT SÂU 3 LƯỢT ĐA-AGENT → `docs/audit-findings-20260622.md`** (110 finding verified: 3 crit, 26 high, 46 med, 35 low + audit-3 scan-only). Đã FIX: ARCH-001 (province area). Lộ trình sửa đề xuất theo rủi-ro:
  ...
  - **DEFER có-lý-do (KHÔNG sửa, rủi-ro>lợi ở scale <10k):** D02 PG-pool (untestable-PG lifecycle risk > lợi ích connect-per-request ở scale này) · N+1 places_in_area (in-memory dict lookup = microseconds, KHÔNG phải DB N+1) · A11Y-01 màu hard-code (đổi hex→token rủi ro lệch màu, LOW) · SEO-03 canonical-month · CWV NuxtImg migration (perf-polish, đụng mọi ảnh — phiên riêng).
