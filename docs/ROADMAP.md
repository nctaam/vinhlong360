...
- ✅ **P1 shared pinned outbound HTTP client, nay bound-complete:** `agent/pinned_http.py` vẫn sở hữu public-address policy, exact-sockaddr dialing, peer verification, TLS hostname/SNI và redirect validation. Mỗi mapped GET (admin image review, auto-learn, crawler, GPT-5.5 quality-burst) nay truyền `EgressPolicy` với encoded/decoded cap riêng, bounded `identity`/`gzip`, DNS admission tối đa 4 daemon lookup thread, và một absolute monotonic deadline cho cả chuỗi.
...
- **Residual egress thật còn lại:** `tests/test_pinned_http_consumers.py::KNOWN_UNPINNED_FETCHERS` tiếp tục ghi nhận các caller bị spec loại trừ: `geocode._query_nominatim`, `realtime.get_weather`, `scheduler._digest_send`, `scheduler._send_telegram_admins` (2 caller cuối là POST). Production behavior vẫn chưa được quan sát vì chưa có deployment được uỷ quyền riêng.
