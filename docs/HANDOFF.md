...
> **STATUS (2026-08-04): active — đã truth-sync.** Bản gốc 2026-06-24; đã cập nhật flow data, baseline regression, remote Git và security/CI remediation. Tranche local mới nhất hoàn tất **Plan A trust/scanner correctness**, **bound-complete pinned egress**, **P1 egress observability**, **P2 bounded consent-cookie redirects**, **crawler exact-origin pinning**, và **Nominatim exact-origin pinning**: `attributes.verifiedAt` là authority duy nhất; scanner repo/package chỉ xét owned immutable inputs; 5 mapped GET có encoded/decoded caps, bounded gzip, four-slot DNS gate, một whole-chain deadline và real-httpcore local-socket tests; redirect crawler/geocode khác origin bị chặn trước resolve/dial; security denial phát đúng một warning origin-only trên logger `security.egress`; request-scoped cookie jar có hard caps. Code/test head hiện tại là `5e8a21c6`; focused pinned + mapped consumers + geocode đạt `351 passed`, changed-file Ruff sạch, `run_hard.py --all` đạt `hard=0`, ratchet không tăng. Full backend run nguyên khối gần nhất vẫn còn 2 maintenance timeout và 1 xdist worker crash không tái hiện; evidence chi tiết ở `docs/ROADMAP.md`. Chưa push/deploy/production mutation; nợ thật còn lại ở §10. Mâu thuẫn với `CLAUDE.md` → CLAUDE.md thắng.
...
- **[P2/out-of-scope] Outbound caller chưa migrate:** `realtime.get_weather`, `scheduler._digest_send`, `scheduler._send_telegram_admins` vẫn ngoài tranche; 2 caller cuối là POST nên không được nới contract GET ngầm để kéo vào.
...
- **[Ops] Production behavior chưa quan sát:** tranche này chỉ verify local qua code/test head `5e8a21c6`; cần lệnh deploy riêng của chủ dự án mới được quan sát health/log trên production. Không push, deploy, secret change, indexing change hoặc production mutation trong tranche.
