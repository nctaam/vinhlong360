...
> **STATUS (2026-07-27): active — đã truth-sync.** Bản gốc 2026-06-24; đã cập nhật flow data, baseline regression, remote Git và security/CI remediation. Tranche local mới nhất hoàn tất **Plan A trust/scanner correctness**, **bound-complete pinned egress**, và **P1 egress observability**: `attributes.verifiedAt` là authority duy nhất; scanner repo/package chỉ xét owned immutable inputs; 3 mapped GET có encoded/decoded caps, bounded gzip, four-slot DNS gate, một whole-chain deadline, real-httpcore local-socket tests, và đúng một warning origin-only trên logger `security.egress` cho mỗi security denial. Candidate local hiện tại `8e4bf9bef3c6c6949c4d22185ca8518591eef276`; fresh coverage đạt `8726` pass, hard `0`, official backend Phase A `8649` + Phase B `284` tests. Chưa push/deploy/production mutation; nợ thật còn lại ở §10. Mâu thuẫn với `CLAUDE.md` → CLAUDE.md thắng.
...
- **[Test baseline] Không còn fail-đã-biết từ nhóm cost-tracker/SEO cũ.** Fresh pre-merge coverage ở candidate `8e4bf9be` đạt `8726 passed, 66 skipped, 26 deselected, 1 xfailed`; official bounded backend đạt Phase A `8649 passed, 58 skipped, 111 deselected, 1 xfailed` và Phase B `284 passed, 19 skipped`. Dùng runner ở §0/§4 và triage mọi failure mới như regression.
...
- **[Ops] Production behavior chưa quan sát:** tranche này chỉ verify local ở candidate `8e4bf9be`; cần lệnh deploy riêng của chủ dự án mới được quan sát health/log trên production. Không push, deploy, secret change, indexing change hoặc production mutation trong tranche.
