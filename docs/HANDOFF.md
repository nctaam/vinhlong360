...
> **STATUS (2026-08-03): active — đã truth-sync.** Bản gốc 2026-06-24; đã cập nhật flow data, baseline regression, remote Git và security/CI remediation. Tranche local mới nhất hoàn tất **Plan A trust/scanner correctness**, **bound-complete pinned egress**, **P1 egress observability**, **P2 bounded consent-cookie redirects**, và **crawler exact-origin pinning**: `attributes.verifiedAt` là authority duy nhất; scanner repo/package chỉ xét owned immutable inputs; 4 mapped GET có encoded/decoded caps, bounded gzip, four-slot DNS gate, một whole-chain deadline, real-httpcore local-socket tests, đúng một warning origin-only trên logger `security.egress` cho mỗi security denial, request-scoped cookie jar có hard caps, và optional exact-origin allowlist chặn redirect khác host/scheme/port trước resolve/dial. Code/test head hiện tại là `d6eee916`; focused crawler+pinned+registry đạt `264 passed`, `run_hard.py --all` đạt `hard=0`, ratchet không tăng. Full backend run nguyên khối còn 2 maintenance timeout và 1 xdist worker crash không tái hiện; evidence chi tiết ở `docs/ROADMAP.md`. Chưa push/deploy/production mutation; nợ thật còn lại ở §10. Mâu thuẫn với `CLAUDE.md` → CLAUDE.md thắng.
