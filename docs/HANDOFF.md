...
> **STATUS (2026-07-30): active — đã truth-sync.** Bản gốc 2026-06-24; đã cập nhật flow data, baseline regression, remote Git và security/CI remediation. Tranche local mới nhất hoàn tất **Plan A trust/scanner correctness**, **bound-complete pinned egress**, **P1 egress observability**, và **P2 bounded consent-cookie redirects**: `attributes.verifiedAt` là authority duy nhất; scanner repo/package chỉ xét owned immutable inputs; 3 mapped GET có encoded/decoded caps, bounded gzip, four-slot DNS gate, một whole-chain deadline, real-httpcore local-socket tests, đúng một warning origin-only trên logger `security.egress` cho mỗi security denial, và request-scoped cookie jar có host/domain/path/`Secure` scope cùng hard caps. Code/test head hiện tại là `c64b7638`; focused pinned+consumer đạt `250 passed`, `run_hard.py --all` đạt `hard=0`, ratchet không tăng. Lệnh official backend regression đã được thử lại nhưng bị giới hạn wrapper 124 giây trước khi Phase A phát kết quả; không ghi nhận failure assertion mới. Chưa push/deploy/production mutation; nợ thật còn lại ở §10. Mâu thuẫn với `CLAUDE.md` → CLAUDE.md thắng.

> Dán toàn bộ file này làm tin nhắn đầu tiên cho phiên/tài-khoản Claude Code mới.
> Cập nhật lần cuối: 2026-07-27. Nhánh chính: `main`; `origin` đã cấu hình. Tranche hiện tại chưa push/deploy và không tái-xác minh health prod.
...
- **[ĐÃ GIẢI local] P1 egress observability:** `PinnedHTTPClient.get()` phát đúng một warning đã sanitize trên `security.egress` cho blocked address, peer mismatch và redirect-policy denial; context cố định là `admin_image_review`, `auto_learn`, `quality_burst`. Không log path/query/fragment/raw exception, không đổi HTTP/return behavior. Production log behavior chưa được quan sát vì chưa deploy.
- **[ĐÃ GIẢI local] P2 cookie-gate:** `PinnedHTTPClient` giữ jar cookie bounded chỉ trong một GET chain, replay `Set-Cookie` cho redirect về cùng URL và vẫn fail-closed với loop không có cookie state mới. Cookie được scope theo host/domain, path và `Secure`; tên/giá trị/header/số lượng đều có cap, không log raw cookie. Regression suite `tests/test_pinned_http.py` đạt `244 passed`; production behavior chưa quan sát.
- **[P2/out-of-scope] Outbound caller chưa migrate:** `crawler.fetch_page`, `geocode._query_nominatim`, `realtime.get_weather`, `scheduler._digest_send`, `scheduler._send_telegram_admins` bị spec GET-only/3-consumer loại trừ; 2 caller cuối là POST nên không được nới contract ngầm để kéo vào.
- **[Ops] Production behavior chưa quan sát:** tranche này chỉ verify local qua code/test head `15f7124a`; cần lệnh deploy riêng của chủ dự án mới được quan sát health/log trên production. Không push, deploy, secret change, indexing change hoặc production mutation trong tranche.
