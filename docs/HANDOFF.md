...
> **STATUS (2026-07-27): active — đã truth-sync.** Bản gốc 2026-06-24; đã cập nhật flow data, baseline regression, remote Git và security/CI remediation. Tranche local mới nhất: **P1 shared pinned outbound HTTP client** (`agent/pinned_http.py`) — 3 đường fetch (admin image review, auto-learn, quality-burst) nay đi qua một biên egress DNS-pinned, redirect-safe; kèm vá lỗ SSRF `fec0::/10`/IPv4-translated/6to4-relay phát hiện bằng audit đối kháng. Chưa push/deploy. Nợ còn lại ở §10. Mâu thuẫn với `CLAUDE.md` → CLAUDE.md thắng.
...
- **[P1 follow-up / Workstream 10] Outbound body + deadline bounds:** shared pinned client (`agent/pinned_http.py`) đã khoá quyền DNS/connect/redirect nhưng CỐ Ý giữ eager buffering và timeout theo TỪNG HOP. Bổ sung bounded decompression/body allocation + 1 monotonic deadline cho cả chuỗi ở tranche riêng. **KHÔNG được nới peer pinning khi làm.** Lưu ý timeout: client áp timeout mỗi hop qua tối đa 6 hop, nên worst-case wall-clock của mỗi consumer đã tăng so với trước migrate.
- **[P1 follow-up] Transport thật chưa từng chạy trong test:** `build_pinned_transport` (default `transport_factory`) chạy trong 0 test — mọi test tiêm `httpx.MockTransport` hoặc monkeypatch `httpcore.ConnectionPool`. Nên `_PinnedNetworkStream` chỉ được kiểm chứng đối chiếu với `FakeSocket`, mà `FakeSocket` viết ra để khớp chính nó. Chưa chạy lần nào: nhánh partial-send `written == 0`, và `get_extra_info("is_readable")` (đưa socket vào `select.select` không guard → socket đã đóng ném `ValueError`, mà `ValueError` KHÔNG nằm trong catch tuple của `_fetch_hop` → thoát ra thô). `_PinnedNetworkBackend.__init__` đã có sẵn tham số `socket_factory`; có thể chạy httpcore thật + stream thật với socket giả trong bộ nhớ, KHÔNG cần network thật.
- **[P1 follow-up] Biên egress không log gì:** địa chỉ bị chặn, peer mismatch (tức đang bị DNS-rebinding/MITM) và redirect bị từ chối đều biến mất không dấu vết. `gpt55_quality_burst.fetch_url_text` còn bị test khoá phải im lặng (`caplog.records == []`). Hệ quả: **một cuộc tấn công SSRF bị chặn thành công trông y hệt một site chậm.** Cân nhắc logger ở `_require_allowed_ip`/`_check_peer_matches_approved`; nhớ sanitize vì địa chỉ là input do kẻ tấn công ảnh hưởng.
- **[P2] Regression cookie-gate do tranche pinned gây ra:** client mới không có cookie jar (đúng thiết kế) VÀ coi URL lặp là loop, nên cổng consent kinh điển `GET /p → 302 /p + Set-Cookie` chết ngay hop đầu, trong khi `httpx`/`requests` trước đây qua được ở hop 2. Quality-burst biến nó thành `(False, "URL could not be fetched")` im lặng — false negative trong luồng verify nguồn.
- **[P2] `verifiedAt` đọc sai trường — VI PHẠM §1.7:** FE `web-nuxt/pages/dia-diem/[id].vue:961` và BE `agent/public_api.py:230` đọc `verifiedAt` top-level (**1746/1746** entity có trong `web/data.json`) thay vì `attributes.verifiedAt` (**0/1746**) mà CLAUDE.md §1.7 quy định là nguồn kiểm-chứng-thực-địa DUY NHẤT → byline hiện "Biên tập & kiểm chứng thực địa" cho MỌI entity. Chưa kiểm DB prod. **Cần chủ dự án quyết** vì là claim đối ngoại.
- **[P2] `test_repository_has_no_duplicate_canonical_artifacts` xung đột với worktree trong repo:** test quét cả cây repo nên mọi worktree dưới `.claude/worktrees/` (harness tạo cho mỗi task spawn) đều làm nó đỏ, kèm `test_unpacked_release_loaders_read_exact_packaged_bytes`. Đã xác minh: `find_duplicate_artifacts()` trả 0 trùng trong cây được track, 100% trùng đến từ worktree tạm. Cân nhắc cho scanner bỏ qua nested worktree.
