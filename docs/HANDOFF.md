...
> **TRUTH-SYNC 2026-09-08 — Task 0 baseline freeze and artifact classification.** Lệnh đo thật lúc `2026-09-08T13:53:23Z`: `git rev-parse HEAD` → `0d537202c381ec75c863612b03ae25d7b1832af0`; `git branch --show-current` → `codex/correction-case-pilot`; `git status --short` ghi nhận 13 tracked frontend/contract edits, `?? agent/.pytest-task15-review-green/`, và `?? outputs/`. `python scripts/check_release_authority.py --root .` → `BLOCKED tracked=7 stale=4 mismatches=1` vì `progress_artifact` trỏ tới per-plan ledger đang ignored/untracked; các tài liệu `CLAUDE.md`, `docs/README.md`, `docs/standards/00-INDEX.md`, và `docs/audit-toan-du-an-2026-08.md` vẫn stale. Đây là baseline trung thực, không phải release evidence. Artifact `agent/.pytest-task15-review-green/` là generated test output và `outputs/` là generated research exports; cả hai đều bị phân loại là untracked/generated, không được stage hoặc đưa vào release bundle, và không bị xoá trong Task 0. Pilot acceptance vẫn **NO_GO**, release verifier vẫn **BLOCKED**.

> **Implementation provenance.** HEAD hiện tại chứa các commit triển khai Tasks 1–6 (`1efe000d`, `61a10742`, `ade34c72`, `7614e490`, `62e8a473`, `89eac4af`, `49a10093`, `ec2b85f3`). Các ô unchecked tương ứng trong plan là stale bookkeeping, không phải bằng chứng rằng code còn thiếu; focused re-verification và production-equivalent evidence vẫn là điều kiện riêng trước khi đổi verdict.
...
4. Chạy `python scripts/ops/run_backend_regression.py --deadline-seconds 7000` để xác lập baseline backend hiện tại (Phase A serial; chỉ `tests/launch_safety/test_closed_installer.py` ở Phase B với `-n 2`). Authority registry ghi nhận baseline chuẩn **15 fail-đã-biết**; mọi failure ngoài roster phải triage như regression.
...
- **[ĐÃ GIẢI local] Nominatim egress:** `_query_nominatim()` không còn dùng `requests.get`; query được encode cố định, đi qua pinned client với cap 64 KiB encoded/256 KiB decoded, deadline toàn chuỗi 15 giây và allowlist đúng origin `nominatim.openstreetmap.org`. Redirect off-origin bị chặn trước resolve/dial; bbox, cache, rate-limit và fallback `None` giữ nguyên. Focused pinned + mapped consumers + geocode đạt `351 passed`.
- **[ĐÃ GIẢI local] OpenWeatherMap egress:** `get_weather()` không còn dùng `httpx.get`; API key được encode trong query nhưng không xuất hiện trong security/transport log, request đi qua pinned client với cap 64 KiB encoded/256 KiB decoded, deadline toàn chuỗi 10 giây và allowlist đúng origin `api.openweathermap.org`. Redirect off-origin bị chặn trước resolve/dial; live cache, seasonal fallback, output shape và circuit-breaker call path giữ nguyên. Focused mapped suite đạt `359 passed`; full resilience `173 passed, 1 skipped`.
- **[P2/out-of-scope] Outbound caller chưa migrate:** chỉ còn `scheduler._digest_send` và `scheduler._send_telegram_admins`; cả hai là Telegram POST, nên không được nới contract GET ngầm để kéo vào.
- **[ĐÃ GIẢI local/Test hygiene] Full-repo Ruff sạch:** `6d2e9d8f` xóa ba import chết dưới `TYPE_CHECKING` trong `tests/checks/test_erasure_lifecycle_complexity.py`; target tuple và complexity assertion giữ nguyên. Focused ratchet `1 passed`, `python -m ruff check .` sạch, `run_hard.py --all` vẫn `hard=0`.
- **[Ops] Production behavior chưa quan sát:** tranche này chỉ verify local qua code/test head `6d2e9d8f`; cần lệnh deploy riêng của chủ dự án mới được quan sát health/log trên production. Không push, deploy, secret change, indexing change hoặc production mutation trong tranche.
