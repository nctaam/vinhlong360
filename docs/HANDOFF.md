...
> **STATUS (2026-07-27): active — đã truth-sync.** Bản gốc 2026-06-24; đã cập nhật flow data, baseline regression, remote Git và security/CI remediation. Tranche local mới nhất hoàn tất **Plan A trust/scanner correctness** và **bound-complete pinned egress**: `attributes.verifiedAt` là authority duy nhất; scanner repo/package chỉ xét owned immutable inputs; 3 mapped GET có encoded/decoded caps, bounded gzip, four-slot DNS gate, một whole-chain deadline, và real-httpcore local-socket tests. Candidate `dab4877163280a6476180e0ad285280e405af1b4` qua focused `303`, frontend `912` sau một run transient exit `1` vì Nuxt setup-hook timeout rồi exact rerun không đổi code/test exit `0`, hard `0`, và official backend Phase A `8633` + Phase B `284` tests. Chưa push/deploy/production mutation; nợ thật còn lại ở §10. Mâu thuẫn với `CLAUDE.md` → CLAUDE.md thắng.
...
- **[Ops] Production behavior chưa quan sát:** tranche này chỉ verify local ở revision `dab48771`; cần lệnh deploy riêng của chủ dự án mới được quan sát health/log trên production. Không push, deploy, secret change, indexing change hoặc production mutation trong tranche.
