...
- [x] **Step 1: Write the failing query-boundary test**
...
- [x] **Step 3: Add the minimal SQL control**
...
- [x] **Step 1: Add failing endpoint and policy tests**
...
- [x] **Step 3: Implement the audience helper and wire the endpoint**
...
- [x] **Step 4: Run GREEN and nearby profile tests**
...
- [x] **Step 1: Add failing backend review-contract tests**
...
- [x] **Step 3: Implement canonical snapshots and stale-review rejection**
...
- [x] **Step 4: Require the token at the admin API boundary**
...
- [x] **Step 5: Add the failing frontend contract test**
...
- [x] **Step 6: Implement the review UI**
...
- [x] **Step 7: Run backend and frontend GREEN**
...
- [x] **Step 5: Review bypass variants and preserve controls**
...
The final independent review found a broader transactional ownership race in `guarded_evolve`: the guarded version is captured only after a potentially long `apply_fn`, so an unrelated edit made during the apply window can become part of the rollback-owned version and be erased. A rollback CAS conflict can also leave the rejected mutation active while the summary still says `decision="rolled_back"`.

These are not additional local Workstream 2 patches. Workstream 6 must introduce an owned atomic commit, a reentrant transaction/session boundary, or targeted three-way rollback; it must also report `rollback_conflict`/`rollback_failed` explicitly. Manual force rollback remains intentionally destructive. POSIX mode/fsync ordering and ownership/ACL durability remain platform-specific follow-up evidence for that workstream.
