...
- Frontend gates from `web-nuxt`: the first exact `npm test` exited `1` with `1 failed | 36 passed` files and `906 passed | 6 skipped` tests after the Nuxt setup hook timed out in `tests/entityStory.test.ts`; with no code/test changes, one exact rerun exited `0` with `37` files and `912` tests passed in `28.66s`. `npm run typecheck` -> exit `0`, no diagnostics; `npm run build` -> exit `0`, `746 modules transformed`, `Σ Total size: 6.45 MB (1.62 MB gzip)`, launch-readiness manifest generated for `dab4877163280a6476180e0ad285280e405af1b4` (existing sourcemap, chunk-size, and Node `DEP0155` warnings remain non-fatal).
