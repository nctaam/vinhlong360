...
> **Progress truth (2026-07-23):** R20.8 and R30.3 are at their committed baselines and `run_hard.py --all` is green on the rollback lane. Rollback/installer recovery is committed in `b5331c4`; Task 45 implementation/evidence commits are on `codex/launch-safety-gate` (`0951048` through `69323cf`) and are not yet integrated into `main`. The installer matrix has `146 passed, 17 skipped, 0 failed` out of `284` collected before its 20-minute bound; `121` nodes remain unrun. Real Nuxt/Chrome acceptance remains pending because the resource-bounded build did not produce `.output/server/index.mjs`. Checkboxes below remain unmarked until clean integration and final evidence are complete.
