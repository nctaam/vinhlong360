...
> **Progress truth (2026-07-23):** Rollback/installer recovery and Task 45 implementation are integrated on clean `codex/ls-remed-rollback` HEAD `9355a8b`. R20.8 and R30.3 are at their committed baselines; `run_hard.py --all` is green (`hard=0`) and focused evidence/browser contracts report `47 passed`. The bounded installer matrix has `146 passed, 17 skipped, 0 failed` out of `284` collected; `121` nodes remain unrun. Real Nuxt/Chrome acceptance remains pending because the resource-bounded build did not produce `.output/server/index.mjs`, so final evidence Markdown and evidence-only Commit B must not be created yet. Nothing is merged into `main`; checkboxes below remain unmarked until the final matrix and evidence are complete.
