...
> **Progress truth (2026-07-23):** Rollback/installer recovery, Task 45 implementation, CI contract wiring, and installer fixture acceptance are integrated on `codex/ls-remed-rollback` through commits `522a3ee` and `0fb3c00`. R20.8 and R30.3 remain at their committed baselines; the latest commit hook reports `run_hard.py --all` green (`hard=0`), focused Task 45 Python contracts report `65 passed`, and the complete installer matrix reports `265 passed, 19 skipped, 0 failed` across `284` nodes. The executable PowerShell contract still requires a clean-HEAD rerun. A resource-bounded Nuxt build and real Chrome browser smoke were previously verified (`.output/server/index.mjs` present; browser harness exit `0`), but that ignored output predates `0fb3c00` and must be rebuilt before revision-bound packaging or final evidence. Final evidence Markdown and evidence-only Commit B remain blocked until the clean PowerShell contract and current-revision build/browser acceptance are green. Nothing is merged into `main`; checkboxes below remain unmarked until final evidence is complete.
