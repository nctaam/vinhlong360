...
Regression-validated source: ae5a7143; subsequent commits update verification documentation only.
...
- Phase A regression command — `python -m pytest -q --ignore=tests/launch_safety/test_closed_installer.py` — PASS at `ae5a7143`: 8,892 passed, 183 skipped, 111 deselected, 1 xfailed, 1 warning; exit code 0; elapsed 21:05.
- Phase B launch-safety command — `python -m pytest -q tests/launch_safety/test_closed_installer.py -n 2 --dist=load --max-worker-restart=0` — PASS at `ae5a7143`: 284 passed, 19 skipped; exit code 0; elapsed 1:29:24.
- The Phase A/B commands were run independently after the test-only commits `d63c6e82` and `ae5a7143`; no production files were changed by those commits.
