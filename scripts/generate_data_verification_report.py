...
DEFAULT_OUTPUT_DIR = SCRATCH_DIR / "data-verification"
WEB_CACHE = SCRATCH_DIR / "data-verification-web-log.json"

ARTIFACT_FILENAMES = {
    "report": "data-verification-report.md",
    "matrix": "data-verification-matrix.csv",
    "claims": "data-verification-claims.csv",
    "web_log": "data-verification-web-log.csv",
    "sources": "data-verification-sources.csv",
    "fixes": "data-verification-fixes.sql",
}
...
def write_fix_sql(path: Path, errors: list[dict[str, Any]]) -> None:
    lines = [
...
def _write_all_csvs(
    ...
    write_fix_sql(artifact_paths["fixes"], CONFIRMED_ERRORS)
...
def _report_core(
    ...
    _write_all_csvs(matrix, claims, web_log, source_rows, artifact_paths)
...
def _render_report(ctx: dict[str, Any]) -> str:
    ...
    artifact_paths = {
        name: _display_path(path) for name, path in ctx["artifact_paths"].items()
    }
...
- Full entity matrix: `{artifact_paths["matrix"]}`
- Full claim inventory: `{artifact_paths["claims"]}`
- Full web cross-reference log: `{artifact_paths["web_log"]}`
- Full source URL audit: `{artifact_paths["sources"]}`
- P0 quarantine SQL: `{artifact_paths["fixes"]}`
...
Full matrix: `{artifact_paths["matrix"]}`. Lowest-trust 60 rows:

{md_table(["entity_id", "type", "area", "trust_score", "verdict", "source_status", "flags"], lowest_rows)}

### B. All Factual Claims Inventory
Full claim inventory: `{artifact_paths["claims"]}` ({len(claims)} extracted claims). Sample:
...
### F. Source URL Audit
Full source audit: `{artifact_paths["sources"]}`.
...
Full log: `{artifact_paths["web_log"]}` ({len(web_log)} searches). Sorted sample:
...
def generate_report(
    ...
    selected_output_dir = Path(output_dir) if output_dir is not None else DEFAULT_OUTPUT_DIR
    selected_output_dir.mkdir(parents=True, exist_ok=True)
    artifact_paths = {
        name: selected_output_dir / filename
        for name, filename in ARTIFACT_FILENAMES.items()
    }
    ctx = _report_core(data, web_log, artifact_paths)
    ...
    return list(artifact_paths.values())
...
def main() -> int:
    ...
    for path in generate_report(data, web_log, output_dir=args.output_dir):
        print(f"Wrote {path}")
    return 0
