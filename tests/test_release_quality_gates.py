...
def test_migration_gate_static_contracts_pass_current_repo():
    ...
    assert errors == []
    assert stats["latest"] == "071_restore_entity_rating_triggers.sql"
    assert stats["latest_schema_version"] == 71
...
def test_perf_quality_trend_migration_contracts_exist():
    ...
    assert "PG_REQUIRED_SCHEMA_VERSION = 71" in database
...
def test_apply_migrations_runner_uses_legacy_baseline_and_latest_plan():
    ...
    assert migrations[-1].path.name == "071_restore_entity_rating_triggers.sql"
