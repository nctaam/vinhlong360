...
def test_migration_gate_static_contracts_pass_current_repo():
    ...
    assert errors == []
    assert stats["latest"] == "073_erasure_delete_actions.sql"
    assert stats["latest_schema_version"] == 73
...
def test_perf_quality_trend_migration_contracts_exist():
    ...
    assert "PG_REQUIRED_SCHEMA_VERSION = 73" in database
...
def test_apply_migrations_runner_uses_legacy_baseline_and_latest_plan():
    ...
    assert migrations[-1].path.name == "073_erasure_delete_actions.sql"
    assert pending_after_legacy_baseline == [
        ...
        "071_feedback_receipts.sql",
        "072_account_erasure_state.sql",
        "073_erasure_delete_actions.sql",
    ]
