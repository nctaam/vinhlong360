...
def test_migration_gate_static_contracts_pass_current_repo():
    ...
    assert errors == []
    assert stats["latest"] == "073_location_preference_remediation.sql"
...
def test_apply_migrations_runner_uses_legacy_baseline_and_latest_plan():
    ...
    assert migrations[-1].path.name == "073_location_preference_remediation.sql"
    assert pending_after_legacy_baseline == [
        ...
        "071_identity_location_preferences.sql",
